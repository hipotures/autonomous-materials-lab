...
The repository now contains both local property / chemistry screens and a trajectory-level evaluator. The entry evaluator includes a [V1 angular surface model and numerical verification study](experiments/entry-evaluator/V1.md) plus a [V2 physical-sensitivity study](experiments/entry-evaluator/V2.md). V1 establishes numerical consistency under fixed assumptions; V2 varies the dominant low-fidelity physical assumptions in matched multi-fluid scenarios and reports whether the ranking survives. Neither establishes flight-level physical validity.
