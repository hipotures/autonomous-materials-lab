...
- [Constrained inverse molecular search V5c](experiments/molecular-search-v5c/README.md)
- [High-throughput molecular search V5d](experiments/molecular-search-v5d/README.md)
- [Active-learning calibration expansion V5e](experiments/active-learning-v5e/README.md)
...
The repository now contains local property / chemistry screens, a trajectory-level evaluator, and a structure-derived property-prediction track. The staged entry work includes [V1 numerical verification](experiments/entry-evaluator/V1.md), [V2 physical-sensitivity screening](experiments/entry-evaluator/V2.md), [V3 cross-model Earth aeroheating comparison](experiments/entry-evaluator/V3.md), [V4 ignition-delay constrained hydrogen cooling](experiments/entry-evaluator/V4.md), and [V5a known binary liquid-mixture modeling](experiments/entry-evaluator/V5a.md). V5b adds SMILES -> GC-PC-SAFT/Joback -> PropertyProvider -> entry evaluator; V5b-2 adds a broad blind holdout and structural applicability-domain uncertainty; V5b-2.1 adds failure taxonomy, repeated-split robustness and leave-one-family-out challenges. V5c-1 then performs deterministic constrained molecular generation and uncertainty-aware ranking against Water. V5c-2 expands the parameter-supported molecular space, adds a thermodynamic property pre-screen, and separates statistically rankable candidates from exploratory new functional families. V5d-1 replaces hand-written enumeration with deterministic graph mutation, evolutionary beam search, diversity preservation, and a hierarchical high-throughput funnel targeting up to 120k new structures. V5e-1 closes the loop with active-learning acquisition: it selects a diverse, property-relevant set of exploratory molecules for independent reference-data resolution and later V5b recalibration; selection alone does not expand the calibrated domain. V5e-2 automates the first reference-resolution handoff using exact PubChem identity checks, exact CoolProp identifier matching, NIST WebBook corroboration, source caching and an explicit calibration-quality gate. V5e-2.1 extends identity-resolved targets beyond CoolProp with an explicit multi-source empirical allowlist over chemicals/thermo, preserves rejected predicted methods in an audit trail, and emits property-calibration anchors for V5b-3 without mislabeling them as entry-level ground truth. None of these reduced-order stages establishes flight-level physical validity.
